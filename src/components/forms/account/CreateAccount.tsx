import React, { useEffect } from "react";
import { CreateAccountForm } from "./CreateAccountForm";
import { post } from "../../../services/api";
import { FormData } from "../../../types/typeFormData";
import { formatarMensagemErro } from "../../../utils/formatarErrors";

import {
  toastCustomSuccess,
  toastCustomError,
  toastCustomAction,
} from "../../ToastCustom";

interface CreateAccountProps {
  isVisible: boolean;
  setIsVisible: React.Dispatch<React.SetStateAction<boolean>>;
}

export const CreateAccount: React.FC<CreateAccountProps> = ({
  isVisible: propIsVisible,
  setIsVisible,
}) => {
  const handleReactivateUser = async (email: string) => {
    toastCustomAction(
      "Usuário",
      email,
      "Conta inativa. Clique para reativar.",
      "Reativar",
      async () => {
        try {
          await post("/usuarios/reativar-por-email", { email });
          toastCustomSuccess("Usuário", email, "Conta reativada com sucesso!");
          setTimeout(() => {
            setIsVisible(false);
          }, 1200);
        } catch (reactivateError) {
          const errorMessage = formatarMensagemErro(reactivateError);
          toastCustomError("Usuário", email, errorMessage);
        }
      },
      "Cancelar",
    );
  };

  const onSubmit = async (formData: FormData) => {
    //  console.log('Formulario enviado', formData);
    const { perfil, dataNascimento, ...commonData } = formData;
    let endpoint = "";
    let dataToSend: Partial<FormData> = commonData;
    // console.log("Payload para /admin:", dataToSend);
    if (perfil === "ALUNO") {
      endpoint = "/alunos";
      dataToSend = { ...commonData, dataNascimento };
      // console.log("Aluno", dataToSend)
    } else if (perfil === "ADMIN") {
      endpoint = "/admin";
      dataToSend = commonData;
      // console.log("Admin", dataToSend)
    } else {
      toastCustomError(
        "Usuário",
        "Perfil inválido. Não foi possível criar o usuário.",
      );
      return;
    }

    try {
      // console.log("Enviando para:", import.meta.env.VITE_BACKEND_BASE_URL + endpoint);
      // console.log("Enviando requisição POST para:", endpoint, "com o payload:", dataToSend);
      const response = await post(endpoint, dataToSend);
      // console.log("Resposta do /admin:", response);
      if (!response || response.status < 200 || response.status >= 300) {
        toastCustomError(
          "Usuário",
          "Admin",
          "A API não confirmou a criação da conta. Tente novamente.",
        );
        return;
      }

      // console.log(response.data);

      const nome = formData.nome || "Usuário";
      toastCustomSuccess("Usuário", nome, "Foi criado com sucesso!!!");

      setTimeout(() => {
        setIsVisible(false);
      }, 1500);
    } catch (error) {
      console.error("Erro na criação:", error);
      const errorMessage = formatarMensagemErro(error);
      const nome = formData.nome || "Usuário";

      if (/inativ|inactiv/i.test(errorMessage)) {
        await handleReactivateUser(formData.email);
        return;
      }

      toastCustomError("Usuário", nome, errorMessage);
    }
  };

  useEffect(() => {
    const handleEscape = (event: KeyboardEvent): void => {
      if (event.key === "Escape") setIsVisible(false);
    };

    const handleBackButton = (): void => {
      setIsVisible(false);
    };

    window.addEventListener("keydown", handleEscape);
    window.addEventListener("popstate", handleBackButton);
    return () => {
      window.removeEventListener("keydown", handleEscape);
      window.removeEventListener("popstate", handleBackButton);
    };
  }, [setIsVisible]);

  if (!propIsVisible) return null;

  return (
    <div className="fixed inset-0 flex items-start justify-center bg-[#FFFFFFB2] z-50">
      <div className="mt-[1rem] md:mt-[1.5rem] font-bold bg-primary1 text-white w-[90%] md:w-[70%] h-[33.5rem] md:h-[36rem] rounded-t-[10px] shadow-2xl">
        <CreateAccountForm onSubmit={onSubmit} setIsVisible={setIsVisible} />
      </div>
    </div>
  );
};
