...
export const initializeLoadingHandler = (
  callback: (isLoading: boolean, message?: string) => void,
) => {
...
api.interceptors.request.use(
  (config) => {
    ...
    // O endpoint de ping recebe um timeout maior para suportar o cold start da API.
    if (config.url?.includes("/ping")) {
      config.timeout = Math.max(config.timeout ?? 0, PING_TIMEOUT_MS);
    }
